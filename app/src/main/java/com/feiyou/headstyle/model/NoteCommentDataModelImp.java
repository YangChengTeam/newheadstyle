...
public class NoteCommentDataModelImp extends BaseModel implements NoteCommentDataModel<NoteCommentRet> {

    private Context context = null;
    private NoteDataServiceApi noteDataServiceApi;
    private CompositeSubscription mCompositeSubscription;

    public NoteCommentDataModelImp(Context mContext) {
        super();
        context = mContext;
    ...
    public void getNoteCommentData(String userId, int page, String msgId, int type, final IBaseRequestCallBack<NoteCommentRet> iBaseRequestCallBack) {
        ...
        mCompositeSubscription.add(noteDataServiceApi.getNoteCommentData(requestBody)  //将subscribe添加到subscription，用于注销subscribe
                .observeOn(AndroidSchedulers.mainThread())//指定事件消费线程
                .subscribeOn(Schedulers.io())  //指定 subscribe() 发生在 IO 线程
                .subscribe(new Subscriber<NoteCommentRet>() {
                    ...
                    @Override
                    public void onNext(NoteCommentRet noteTypeRet) {
                        //回调接口：请求成功，获取实体类对象
