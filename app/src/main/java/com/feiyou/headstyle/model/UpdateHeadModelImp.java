...
public class UpdateHeadModelImp extends BaseNoRsaModel implements UpdateHeadModel<UpdateHeadRet> {
    ...
    @Override
    public void updateBackground(String userId, String filePath, IBaseRequestCallBack<UpdateHeadRet> iBaseRequestCallBack) {
        MultipartBody multipartBody = filesToMultipartBody(userId, filePath);
        mCompositeSubscription.add(userInfoServiceApi.updateBackground(multipartBody)  //将subscribe添加到subscription，用于注销subscribe
                .observeOn(AndroidSchedulers.mainThread())//指定事件消费线程
                .subscribeOn(Schedulers.io())  //指定 subscribe() 发生在 IO 线程
                .subscribe(new Subscriber<UpdateHeadRet>() {
